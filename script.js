    ...
    while (1) {
        ...
        if (tmp[0] == 0) {
            ...
            if (bz != 0) {
                ...
                let y = findPicTimes(5, 200, 0, "小黄", "小红");
                clickp(y[1]);
                let j = findPicTimes(2, 800, 0, "加药", "使用");
                if (j == 0) {
                    let y = findPicTimes(2, 800, 0, "中红");
                    clickp(y[1]);
                    let j = findPicTimes(2, 800, 0, "加药", "使用");
                    if (j == 0) {
                        let y = findPicTimes(2, 800, 0, "大红");
                        clickp(y[1]);
                        let j = findPicTimes(2, 800, 0, "使用");
                        if (j == 0) {
                            qqerror(screen, "你药没啦！");
                            return 0;
                        } else if (j[0] == 0) {
        ...
        if (tl[0] == 0) {
            ...
            while (1) {
                ...
                if (ks[0] == 2) {
                    ...
                    events.broadcast.emit("message2", "续战已结束，开始补体");
                    let js = findPic("ok");
                    clickp(js[1]);
                    { let a = findPic("加体"); if (a == false) return 0; else clickp(a[1]) };
                    let y = findPicTimes(5, 200, 0, "小黄", "小红");
