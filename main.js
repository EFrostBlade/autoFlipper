...
function upDate() {
    const version = "5.2";
    const picNum = 147;
    const baseUrlproxy = "https://gh-proxy.com/raw.githubusercontent.com/EFrostBlade/autoFlipper/main/";
    ...
    if (Number(version) > Number(oldVersion) && Number(oldVersion) <= 5.0) {
        ...
        if (img2 != null) {
            images.save(img2, scriptPath + "/res/" + WIDTH + "/139.png");
            log("保存图片于" + scriptPath + "/res/" + WIDTH + "/139.png");
            img2.recycle();
        }
        let img3 = images.load(downUrl + "res/" + WIDTH + "/140.png");
        if (img3 != null) {
